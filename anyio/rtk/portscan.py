...
#
# Find a serial port for the RTk.GPIO / Arduino board.
...
def autoDetect():
  ...
  if len(devices) == 1:
    ...
    # Multiple devices — can't auto-detect, need interactive
    return None
...
def getName():
  """Read cached port name, or None."""
  try:
    with open(CACHE_NAME, "r") as f:
      name = f.readline().strip()
      if name and os.path.exists(name):
        return name
      # Cached device no longer exists
      return None
...
def find():
  """Find a port: try auto-detect first, then interactive scan."""
  dev = autoDetect()
  if dev is not None:
    remember(dev)
    return dev

  # Multiple or no devices — fall back to interactive
  dev = scan()
  if dev is not None:
    remember(dev)
  return dev
